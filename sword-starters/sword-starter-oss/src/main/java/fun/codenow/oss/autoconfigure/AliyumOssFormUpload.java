 ...
 * @Date2020/8/10 14:11
 **/
public class AliyumOssFormUpload {

    CodenowAliyunOssProperties codenowAliyunOssProperties;
    // 上传文件
     ...
     * @throws Exception
     */
    private void PostObject(CodenowAliyunOssProperties codenowAliyunOssProperties, String objectName, String localFilePath) throws Exception {
        // 在URL中添加存储空间名称，添加后URL如下：http://yourBucketName.oss-cn-hangzhou.aliyuncs.com
        String urlStr = codenowAliyunOssProperties.getEndpoint().replace("http://", "http://" + codenowAliyunOssProperties.getBucketName()+ ".");
        // 表单Map。
        ...
        // 设置OSSAccessKeyId。
        formFields.put("OSSAccessKeyId", codenowAliyunOssProperties.getAccessKeyId());
        String policy = "{\"expiration\": \"2120-01-01T12:00:00.000Z\",\"conditions\": [[\"content-length-range\", 0, 104857600]]}";
        String encodePolicy = new String(Base64.encodeBase64(policy.getBytes()));
        // 设置policy。
        formFields.put("policy", encodePolicy);
        // 生成签名。
        String signaturecom = com.aliyun.oss.common.auth.ServiceSignature.create().computeSignature(codenowAliyunOssProperties.getAccessKeySecret(), encodePolicy);
        // 设置签名。
        formFields.put("Signature", signaturecom);
        String ret = formUpload(urlStr, formFields, localFilePath);
        System.out.println("Post Object [" + objectName + "] to bucket [" + codenowAliyunOssProperties.getBucketName() + "]");
    ...
    public static void main(String[] args) throws Exception {
        AliyumOssFormUpload ossPostObject = new AliyumOssFormUpload();
